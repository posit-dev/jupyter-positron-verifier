"""
LimeLM entitlement check via license-manager subprocess.
...
class EntitlementChecker:
    """Checks LimeLM entitlement via license-manager and caches the result."""
    ...
    def from_env(cls) -> "EntitlementChecker":
        ...
        Configure from environment variables.
          ...
          POSITRON_SKIP_ENTITLEMENT_CHECK  Set to '1' to skip LimeLM checks (dev only).
        """
    ...
    async def is_valid(self) -> bool:
        """Return True if the LimeLM entitlement is valid (uses cache)."""
    ...
    async def _fetch(self) -> EntitlementResult:
        if self._skip:
            logger.warning("Entitlement check skipped (POSITRON_SKIP_ENTITLEMENT_CHECK=1)")
            return EntitlementResult(valid=True, licensee="Development")

        if not self._path:
            logger.error(
                "POSITRON_LICENSE_MANAGER_PATH not set; cannot verify LimeLM entitlement"
            )
        ...
        try:
            ...
            if status in ("activated", "evaluation"):
                ...
                logger.info(f"LimeLM entitlement valid: status={status}, licensee={licensee}")
                return EntitlementResult(valid=True, licensee=licensee)
            else:
                logger.error(f"LimeLM entitlement invalid: {data}")
                return EntitlementResult(valid=False)
